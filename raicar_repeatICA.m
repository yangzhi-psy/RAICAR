...
try
    ...
    nii = load_nii (subj.setup.inputNm);
    orig = nii.img;
    subj.result.TR = nii.hdr.dime.pixdim(5);
    subj.result.info = nii.hdr;
    fprintf ('success\n');
catch
...
try
    ...
    nii = load_nii (subj.setup.mskNm);
    subj.result.mask = nii.img;
    clear nii;
    fprintf ('success\n');
catch
...
save prepareResult.mat subj

% assign the processing tasks
if phase == 1
    % conduct multiple ICA
    fprintf ('\n ================ Phase 1 started ================\n');  
    subj.setup.rlPrefix = strcat (subj.setup.outPrefix, '_ICA');
...
%% save result
%fprintf ('\n saving results ...\t');
%nii.hdr = subj.result.info;
%nii.hdr.dime.dim(5) = size (subj.result.aveTc, 2);
%nii.img = subj.result.aveMap;
%
  %%%%%%%%% July 24, 2007 LPI orientation %%%%%%%%%%%%%%%%
  % flip the image to cooperate the LPI coordinate in NIFTI %
  % after writing out it's necessary to call AFNI's nifti_tool
  % to modify the nifti header                              %
%nii.img = flipdim (nii.img, 1);
%nii.img = flipdim (nii.img, 2);
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

  %%%%%%%%  July 24, 2007 writing float format %%%%%%%%%%%%
  % write the component map in float type to avoid the    %
  % cut-off error                                         %
%nii.hdr.dime.datatype = 16;
%nii.hdr.dime.bitpix   = 32;
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%save_nii (nii, [subj.setup.outPrefix, '_aveMap', '.nii']);
%clear nii;
fprintf ('seccess\n');

% prepare result
%[subj] = raicar_prepareShow (subj, 5);

% generate web page report
%if subj.setup.webReport ~= 0 
%    raicar_generateWebReport (subj);
%end
