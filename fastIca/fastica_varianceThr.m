function [Out1, Out2, Out3, flagConverge] = fastica_varianceThr(mixedsig, varargin)
...
lastEig           = Dim;
%interactivePCA    = 'off';
varianceThr		  = 0.95; % variance explained in PCA
...
if (rem(length(varargin),2)==1)
  ...
  for i=1:2:(length(varargin)-1)
    ...
    switch lower (varargin{i})
	 ...
	 case 'variancethr' % added argin: percentage of variance explained by kept PCs
		varianceThr = varargin{i+1};
     case 'interactivepca'
...
%    [E, D]=pcamat(mixedsig, firstEig, lastEig, interactivePCA, verbose);
    [E, D]=pcamat_varianceThr(mixedsig, varianceThr, verbose);
  end
end
...
if only > 2
  ...
  [A, W,flagConverge] = fpica (whitesig,  whiteningMatrix, dewhiteningMatrix, approach, ...
